...
content = """use sysinfo::{Components, System};
use std::fs;
...
pub fn trigger_macro(mac: &MacroDef) {
    match mac.kind.as_str() {
        "cmd" => {
            let _ = Command::new("sh").arg("-c").arg(&mac.value).spawn();
        }
        "keys" => {
            let _ = Command::new("xdotool").arg("key").arg(&mac.value).spawn();
        }
        "text" | "chat" => {
            if Command::new("wtype").arg(&mac.value).spawn().is_err() {
                let _ = Command::new("xdotool").arg("type").arg("--delay").arg("10").arg(&mac.value).spawn();
            }
            if Command::new("wtype").arg("-k").arg("Return").spawn().is_err() {
                let _ = Command::new("xdotool").arg("key").arg("Return").spawn();
            }
        }
        _ => {}
    }
...
with open("/home/andre/Desktop/Projects/SysMon/sysmon-server/src/sys.rs", "w") as f:
    f.write(content)
